...
vows.describe('new EventedLoop').addBatch({
	...
	'when an interval is removed using .remove()': {
		topic: function () {
			var loop = new EventedLoop();
			loop.on('400ms', function (e) {});
			loop.on('400ms', function (e) {});
			var toRemove = loop.on('200ms', function (e) {});
			toRemove.remove();
			return loop.intervalLength;
		},
	...
	'when a few complicated intervals are removed using .remove()': {
		topic: function () {
			var loop = new EventedLoop();
			loop.on('400ms', function (e) {});
			var r1 = loop.on('400ms', function (e) {});
			loop.on('420ms', function (e) {});
			var r2 = loop.on('1s', function (e) {});
			var r3 = loop.on('200ms', function (e) {});
			r1.remove();
			r2.remove();
			r3.remove();
			return loop.intervalLength;
